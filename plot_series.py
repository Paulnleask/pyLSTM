...
To run (for example PLTR stock):
    python plot_series.py --csv data/NVDA.csv --artifacts artifacts --outdir forecasts
"""
...
def main():
    ...
    ap.add_argument("--outdir", default="forecasts", type=str)
    args = ap.parse_args()
    ...
    # ---------------- Load stock historical data csv ----------------
    ...
    # ---------------- Plot historical, training and forecasted stock data ----------------
    ...
    # Plot vertical line where training stops
    if "Date" in df_all.columns:
        cutoff_date = df_all["Date"].iloc[cutoff_idx_price - 1]
        plt.axvline(cutoff_date, color="grey", alpha=0.5, linestyle=":")
        plt.text(cutoff_date, plt.ylim()[1], " training", va="top", ha="right", fontsize=9, color="grey")
    ...
    plt.legend()
    plt.tight_layout()
    out_png = outdir / f"Forecast_{csv_title}_W{cfg['seq_len']}_H{cfg['horizon']}_D{cfg['num_layers']}.png"
